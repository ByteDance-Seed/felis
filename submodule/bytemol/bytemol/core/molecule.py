...
from bytemol.core.conformer import (
    Conformer,
    is_charge_key,
    is_energy_key,
    is_force_key,
...
def assert_good_molecule(mol: Molecule,
                         check_halogen_formal_charge: bool = False,
                         check_halogen_connectivity: bool = False) -> None:
    '''
    a 'good' molecule satisfy:
    1. is an individual molecule, '.' does not appear in smiles
    2. elements covered by [H, B, Si, C, N, O, F, P, S, Cl, Br, I]
    3. (optional) formal charge for [F, Cl, Br, I] must be non-positive
    4. (optional) halogen connectivity must be 1
    5. hybridization in S, SP, SP2, SP3

    Note: rdkit parses Chlorate ClO3- to [O-:1][Cl+2:2]([O-:3])[O-:4]

    '''
    smi = mol.get_smiles(isomeric=True)
    assert '.' not in smi, f'not an individual molecule: {smi}'

    good_elements = set([1, 5, 6, 7, 8, 9, 14, 15, 16, 17, 35, 53])
    assert set(mol.atomic_numbers).issubset(good_elements), f'bad elements found: {mol.atomic_numbers}'

    if check_halogen_formal_charge:
    ...
    if check_halogen_connectivity:
        ...
        for idx, num in enumerate(mol.atomic_numbers):
            if num in [9, 17, 35, 53]:
                assert connectivity[idx] == 1, \
                    f'bad connectivity: idx {idx} atomic number {num} is X{connectivity[idx]}'
