...
assert version.parse(ase.__version__) >= version.parse('3.24.0')
...
class Conformer:
    ...
    def from_ase_atoms(cls, atoms: ase.Atoms, confdata: dict = None):
        ...
        # ase 3.23 and later may put info in atoms.calc.results
        try:
            for k, v in atoms.calc.results.items():
                if is_force_key(k):
                    confdata[k] = unit.eV_A_to_kcal_mol_A(v)
                if is_energy_key(k):
                    confdata[k] = unit.eV_to_kcal_mol(v)
    ...
    def from_xyz(cls, xyz_file: str):
        atoms_list = aio.read(xyz_file, index=':', properties_parser=xyz_properties_parser)
    ...
    def to_xyz(self, filename: Union[str, io.StringIO], append=False, confkeys=None):
        ...
        if isinstance(filename, io.StringIO):
            aio.write(filename, atoms, format='extxyz', append=append, write_info=True, write_results=True, plain=False)
        else:
            with open(filename, io_flag) as f:
                aio.write(f, atoms, format='extxyz', append=append, write_info=True, write_results=True, plain=False)
